...
inValidation <- createDataPartition(y=remaining$classe, p=0.50, list=FALSE)
...
TrainAccurracies <- matrix(c(Acc_Tree,Acc_RF,Acc_lda,Acc_gbm,Acc_nb,AccPP_Tree,AccPP_RF,AccPP_lda,AccPP_gbm,AccPP_nb), ncol = 5)
...
TestAccurracies <- matrix(c(Acc_Tree,Acc_RF,Acc_lda,Acc_gbm,Acc_nb,AccPP_Tree,AccPP_RF,AccPP_lda,AccPP_gbm,AccPP_nb), ncol = 5)
...
Now one can see, even based on the accuracy in the training set, classification tree without preprocessing, random forest without preprocessing, linear discriminant analysis (with or without preprocessing), naive bayes with preprocessing and gradient boosting method with preprocessing can already eliminated due to worse results. Naive bayes without preprocessing gives the highest accuracy for the testing set and is thus considered to be the best model. Hence, this model will be used to predict the output based on the given test set. First the out of sample error is calculated using the validation set:

```{r,results="hide", warning=FALSE, message=FALSE}
Pred_nb <- predict(Model_nb, newdata=validation);Acc_nb <- sum(Pred_nb == validation$classe)/length(Pred_nb)
OutOfSampleError <- 1-Acc_nb
```
```{r}
OutOfSampleError
```

Now the given test set can be loaded and the model can be used to make the predictions based on this test set.
```{r,results="hide", warning=FALSE, message=FALSE}
pmltesting = read.csv("pml-testing.csv", na.strings=c("NA",""))
Pred_nb <- predict(Model_nb, newdata=pmltesting)
```
```{r}
Pred_nb
```
